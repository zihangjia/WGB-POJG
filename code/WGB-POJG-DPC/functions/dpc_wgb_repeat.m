function metric_ave = dpc_wgb_repeat(data_matrix, label, cluster_num, wgb_num, options, repeat_num, metric_name)
%BSC_WGB_REPEAT  多次运行 BSC-WGB 聚类算法并返回平均性能指标
%
%   metric_ave = BSC_WGB_REPEAT(data_matrix, label, cluster_num, wgb_num, options, repeat_num, metric_name)
%
%   输入参数:
%   ------------------------------------------------------------
...
%
    ...
    for item = 1:repeat_num
        ...
        % 运行一次 dpc-PGB 聚类算法
        label_pred = dpc_wgb(data_matrix, cluster_num, wgb_num, options);
        % 根据指定指标计算性能
